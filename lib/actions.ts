...
const rows = await sqlSELECT id FROM users LIMIT 1;
if (rows.length > 0) return Number(rows[0].id);
const created = await sqlINSERT INTO users (name) VALUES ('Estudante') RETURNING id;
...
const existing = await sql  SELECT MAX(id) as id FROM study_weeks   WHERE user_id = ${userId} AND week_start = ${weekStartStr}  ;
...
const prevWeek = await sql  SELECT MAX(id) as id, MAX(streak) as streak FROM study_weeks   WHERE user_id = ${userId} AND week_start = ${prevWeekStr}  ;

let streak = 0;
const prevWeekId = prevWeek[0]?.id ? Number(prevWeek[0].id) : null;
if (prevWeekId) {
const prevWeekDays = await sql  SELECT COUNT(*) as count FROM study_days   WHERE week_id = ${prevWeekId} AND completed = TRUE  ;
...
await sql  INSERT INTO study_weeks (user_id, week_start, streak)   VALUES (${userId}, ${weekStartStr}, ${streak})   ON CONFLICT DO NOTHING  ;

const weekRow = await sql  SELECT MAX(id) as id FROM study_weeks   WHERE user_id = ${userId} AND week_start = ${weekStartStr}  ;
const weekId = Number(weekRow[0].id);

// Only insert days/tasks if this week has none yet
const existingDays = await sql  SELECT COUNT(*) as count FROM study_days WHERE week_id = ${weekId}  ;
if (Number(existingDays[0].count) > 0) return weekId;

for (const day of WEEKLY_PLAN) {
const newDay = await sql  INSERT INTO study_days (week_id, day_name, focus, total_minutes)   VALUES (${weekId}, ${day.dayName}, ${day.focus}, ${day.totalMinutes})   RETURNING id  ;
const dayId = Number(newDay[0].id);
for (const task of day.tasks) {
await sql  INSERT INTO tasks (study_day_id, title, minutes)   VALUES (${dayId}, ${task.title}, ${task.minutes})  ;
}
}
...
const weekRows = await sqlSELECT * FROM study_weeks WHERE id = ${weekId};
const week = weekRows[0];

const dayRows = await sql  SELECT * FROM study_days WHERE week_id = ${weekId}   ORDER BY CASE day_name   WHEN 'Segunda' THEN 1   WHEN 'Terça'   THEN 2   WHEN 'Quarta'  THEN 3   WHEN 'Quinta'  THEN 4   WHEN 'Sexta'   THEN 5   WHEN 'Sábado'  THEN 6   ELSE 7   END  ;

const taskRows = await sql  SELECT t.* FROM tasks t   INNER JOIN study_days sd ON t.study_day_id = sd.id   WHERE sd.week_id = ${weekId}  ;
...
await sqlUPDATE tasks SET completed = ${completed} WHERE id = ${id};

const taskRows = await sqlSELECT study_day_id FROM tasks WHERE id = ${id};
if (taskRows.length === 0) return;

const dayId = Number(taskRows[0].study_day_id);
const tasks = await sqlSELECT completed FROM tasks WHERE study_day_id = ${dayId};
const allDone = tasks.length > 0 && tasks.every((t) => Boolean(t.completed));
await sqlUPDATE study_days SET completed = ${allDone} WHERE id = ${dayId};

const dayRows = await sqlSELECT week_id FROM study_days WHERE id = ${dayId};
if (dayRows.length > 0) {
const weekId = Number(dayRows[0].week_id);
const wDays = await sqlSELECT completed FROM study_days WHERE week_id = ${weekId};
const completedCount = wDays.filter((d) => Boolean(d.completed)).length;
await sqlUPDATE study_weeks SET streak = ${completedCount} WHERE id = ${weekId};
}
}